...
function SymbolList({handleSymbol,selectedSymbol}){
    ...
    return(
        <>
            ...
            <div onClick={handleDropDown} className='mx-auto w-32 border rounded-xl mb-2 flex justify-center space-x-2 text-white text-center font-bold'><div>{selectedSymbol} </div><img src={`${downArrow}`}/></div>
            
            {isDropDownActive&&symbols.map(({symbol},idx)=>{
                return <div className='mx-auto w-full   text-white text-center pr-3 opacity-75 hover:opacity-100' key={idx} onClick={()=>{handleSymbol(symbols[idx]); handleDropDown() }}>{symbol}</div>
                })}
            </div>
        </>
    )
